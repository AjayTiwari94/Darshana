...
const StorytellingHub: React.FC = () => {
        ...
        <AnimatePresence>
          {hoveredStory === story._id && (
            <motion.button
              ...
              className="absolute top-2 left-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-2 rounded-full shadow-lg"
              onClick={(e) => {
                e.stopPropagation()
                // Start a new session if one doesn't exist
                if (messages.length === 0) {
  ...
  const CulturalStoryCard: React.FC<{ story: CulturalStory }> = ({ story }) => (
    <motion.div 
      className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all cursor-pointer relative"
      whileHover={{ y: -5, scale: 1.02 }}
      initial={{ opacity: 0, y: 20 }}
    ...
    >
      <div className="relative">
        ...
        {/* Floating AI Button for cultural stories */}
        <AnimatePresence>
          {hoveredStory === story.id && (
            <motion.button
              ...
              onClick={(e) => {
                ...
                // Set context for the current story
                const storyContext = `The user is interested in the cultural story: ${story.title}. ${story.description}`
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          <Link 
            ...
            className="inline-flex items-center px-3 py-1 bg-gradient-to-r from-amber-500 to-orange-600 text-white text-sm font-medium rounded-lg hover:opacity-90 transition-opacity"
          >
            Explore
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <motion.div 
            ...
            transition={{ duration: 0.5, delay: 0.4 }}
          >
            <Link href="/cultural-stories" className="bg-white bg-opacity-20 backdrop-blur-sm rounded-xl p-6 hover:bg-opacity-30 transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'all' ? (
          <>
            ...
            {featuredStory && (
              <motion.div 
                className="bg-white shadow-sm border-b mb-8 rounded-xl overflow-hidden"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5 }}
              >
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        <img
                          src={featuredStory.mediaAssets.images[currentImageIndex] || '/placeholder-story.jpg'}
                          alt={featuredStory.title}
                          className="w-full h-64 lg:h-96 object-cover rounded-lg"
                        />
                        
                        {featuredStory.mediaAssets.images.length > 1 && (
                          <>
                            <button
                              onClick={prevImage}
                            ...
                            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                              {featuredStory.mediaAssets.images.map((_, index) => (
                                <button
                                  key={index}
                      ...
                      {/* Audio Player */}
                      {featuredStory.mediaAssets.audio && (
                        <div className="bg-gray-100 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-gray-700">Audio Narration</span>
                            <span className="text-sm text-gray-500">
                              {formatDuration(mediaPlayer.currentTime)} / {formatDuration(featuredStory.duration)}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1 bg-gray-300 rounded-full h-2">
                              <div
                                ...
                                style={{ width: `${(mediaPlayer.currentTime / featuredStory.duration) * 100}%` }}
                              />
                            </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center mb-4">
                          <img
                            src={featuredStory.narrator.avatar || '/placeholder-avatar.jpg'}
                            alt={featuredStory.narrator.name}
                            className="h-12 w-12 rounded-full mr-3"
                          />
                          <div>
                            <p className="font-medium text-gray-900">{featuredStory.narrator.name}</p>
                            <p className="text-sm text-gray-500">{featuredStory.narrator.bio}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 py-4 border-t border-b border-gray-200">
                        <div>
                          <p className="text-sm text-gray-500">Monument</p>
                          <p className="font-medium">{featuredStory.monument.name}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-500">Period</p>
                          <p className="font-medium">{featuredStory.period}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-500">Category</p>
                          <p className="font-medium">{featuredStory.category}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-500">Duration</p>
                          <p className="font-medium">{formatDuration(featuredStory.duration)}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-4">
                        <button
                          onClick={() => toggleLike(featuredStory._id)}
                        ...
                        >
                          ...
                          <span>{featuredStory.statistics.likes}</span>
                        </button>

                        <button
                          onClick={() => toggleBookmark(featuredStory._id)}
                          className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors ${
                        ...
                        <button
                          onClick={() => shareStory(featuredStory)}
                        ...
                        {/* Talk to Narad AI button for featured story */}
                        <button
                          onClick={() => {
                            // Start a new session if one doesn't exist
                            if (messages.length === 0) {
                            ...
                            // Set context for the current story
                            const storyContext = `The user is reading about: ${featuredStory.title}. ${featuredStory.description}`
                            
                            // Open the AI chat
                            setNaradAIOpen(true)
                          }}
                          className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg hover:opacity-90 transition-opacity"
                        >
                      ...
                      <div className="flex items-center space-x-6 text-sm text-gray-500">
                        <span>{featuredStory.statistics.views.toLocaleString()} views</span>
                        <span className="flex items-center">
                          <StarIcon className="h-4 w-4 mr-1" />
                          {featuredStory.statistics.averageRating.toFixed(1)} ({featuredStory.statistics.totalRatings} ratings)
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Discover Stories</h2>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {getCulturalStoriesByType().map((story) => (
                <CulturalStoryCard key={story.id} story={story} />
              ))}
            </div>

            {/* Places Section */}
            <div className="mt-12" id="cultural-stories-section">
